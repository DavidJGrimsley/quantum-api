...
3. Confirm staging secrets are set: `DATABASE_URL`, `SUPABASE_URL`, `REDIS_URL`, `METRICS_TOKEN`, and `API_KEY_HASH_SECRET` (non-default when `AUTH_ENABLED=true` in staging).
