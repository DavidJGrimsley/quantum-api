...
3. Confirm staging secrets are set: `API_KEYS_JSON`, `REDIS_URL`, `METRICS_TOKEN`.
4. Confirm `APP_ENV=staging` and `ALLOW_ORIGINS` is explicit (no `*`).
...
3. Run smoke tests:
   ...
   - `GET /v1/echo-types` with valid API key returns `200` and `RateLimit-*` headers.
